// 裸 ATmega328P-PU: CHRアドレス A0-A11 と CHRデータ下位5bit。I2Cスレーブ 0x21。
//
// ■ CHR A12(PB0/14番)はもう使わない
// PB0 は High/Low を命令しても電圧が動かない故障が確定した(カートを外した
// 単体状態でも症状が出たため、チップ内部の出力段の故障と判断)。
// A12 は Uno の A3(PC3、カート55番)へ移設した。詳細は fc_uno.ino のコメント参照。
// このファイルでは PB0 に一切触らない。
// ★ 配線側の作業として、328Pの14番→カート55番の線は忘れずに外すこと。
//   繋いだままだと、故障して不安定な PB0 と Uno の A3 が同じカート55番を
//   同時に駆動する形になり、Unoが勝つ見込みは高いが保証はできない。
//
// ■ 水晶の周波数に依存しない
...
static void setActive(uint8_t on) {
  if (on) {
    ...
    DDRD  = 0xFF;                // アドレスは出力
    DDRC |= 0x0F;
    // PB0(旧CHR A12)はもう使わない。入力のまま放置してよい(配線も外す)。
  } else {
